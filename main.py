...
from raw_image_preprocessing import *

# its the solution for url_lib error
import ssl
...
gener_batch_size = 1  # Batch size for generator
dis_batch_size = 1  # Batch size for discriminator
...
if optimizer == 'Adam':
    ...
    optim_dis = optim.Adam(filter(lambda p: p.requires_grad, discriminator.parameters()), lr=lr_dis,
                           betas=(beta1, beta2))
elif optimizer == 'SGD':
    ...
    optim_dis = optim.RMSprop(filter(lambda p: p.requires_grad, discriminator.parameters()), lr=lr_dis, eps=1e-08,
...
def compute_gradient_penalty(D, real_samples, fake_samples, phi):
    ...
    alpha = torch.Tensor(np.random.random((real_samples.size(0), 1, 1, 1)), device=real_samples.device)
    # Get random interpolation between real and fake samples
    interpolates = (alpha * real_samples + ((1 - alpha) * fake_samples)).requires_grad_(True)
    d_interpolates = D(interpolates)
    fake = torch.ones([real_samples.shape[0], 1], requires_grad=False, device=real_samples.device)
...
def train(noise, generator, discriminator, optim_gen, optim_dis,
    ...
    train_set = train_dl #torchvision.datasets.CIFAR10(root='./data', train=True, download=False, transform=transform)
    ...
    for index, (img, _) in enumerate(train_loader):
        ...
        noise = torch.FloatTensor(
            np.random.normal(0, 1, (img.shape[0], latent_dim)))  # noise(img, latent_dim)#= args.latent_dim)
